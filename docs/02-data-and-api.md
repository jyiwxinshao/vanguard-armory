...
此清单同时包含已实现接口与后续计划。目前 `/api/health`、`/api/meta`、`/api/equipments`、`/api/equipments/:id`、四个 `/api/auth` 接口及 `/api/admin/me` 已接入。不支持的装备查询参数返回 422，其他未实现接口返回 404。
...
| 方法 | 路径 | 输入或返回要点 |
| --- | --- | --- |
| POST | `/api/orders` | 整车确认条目和兑换信息；成功返回新订单及快照明细 |
...
```json
{
  ...
  "remark": "",
  "items": [
    { "equipment_id": 1, "quantity": 2, "expected_price": 12900 },
    { "equipment_id": 5, "quantity": 1, "expected_price": 3900 }
  ]
}
```

`expected_price` 是确认时价格，用于提示变价，不能作为计价来源。后端要求 items 与当前服务器整车内容一致，且拒绝重复 ID；所有金额取锁定后的数据库价格。该示例在价格未变化、库存足够时由服务器算出 total=29700、discount=0、actual_total=29700。

支付或取消必须检查归属。重复支付一个仍为 paid 的订单返回当前订单且不改支付时间；重复取消已 cancelled 的订单返回当前订单且不回库存；其他非法转换返回 409。已 completed 订单不能重新支付、取消或回到 paid。
...
3. 锁定用户 carts 行，读取购物车，与请求条目逐项核对 ID 和数量；不一致返回 409。
...
7. 提交成功后返回 201。任何失败都回滚并释放连接，失败时不清空前端或服务器购物车。

服务器购物车写操作同样先锁 carts 行；涉及装备更新时按同一装备 ID 顺序处理。第二个同时下单请求只能看到空车或内容变化，不会再对同一整车重复扣库存。这个约束不等价于完整的请求幂等机制：响应丢失时前端应先刷新购物车和订单记录，不自动重发创建订单。
...
重复取消只有读取已取消状态的结果，没有第二次库存增加。管理员取消调用相同服务；完成订单仅允许 paid → completed。支付与取消竞争失败的一方读取新状态后返回 409。
