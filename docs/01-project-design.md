...
本方案以一个人完成课程开发、测试、文档和答辩为前提。第一版围绕游戏装备浏览、购买和交付形成完整流程，优先保证数据正确和页面可用；基础验收通过后，再选择少量视觉或交互扩展。任务书仅作参考，明显错误和无关内容直接忽略，具体功能按游戏场景与单人开发规模确定。本文描述完整设计；当前用户端已接通认证、装备浏览、购物车与订单，管理端尚未实现；具体范围与验证状态见[第五阶段记录](08-stage-five.md)。
...
用户已自行修改首页为 Vanguard Armory，后续开发以当前页面和 [DESIGN.md](DESIGN.md) 为视觉依据，保留品牌、四列卡片、筛选结构、导航、配色和现有组件，不恢复旧列表布局，也不建立新的视觉语言。详情、购物车、订单与认证页面继续使用同一套设计。管理后台尚未实现，未来可使用 Element Plus 表格结构，但保留同一深色配色与品牌。
...
未决批次绑定原账号，退出后新的游客选购使用另一批次，不能并入其他账号。本地购物车使用带版本的批次结构和 Web Locks 互斥写入；跨标签页通过 storage 事件刷新，事件发生在请求期间时会在请求完成后补刷新。所有服务器请求及排队操作校验账号会话和购物车状态版本，旧成功响应、错误及 finally 均不能影响新账号。合并被 422 拒绝时可以先整理服务器购物车再以原批次重试；结果未知时先恢复合并再写入。
...
工程基础、六张业务表和合并记录表、演示数据、认证、装备浏览、游客与服务器购物车及登录合并已接入。当前保留用户修改后的 Vanguard Armory 页面；订单确认、模拟支付、完整后台、正式素材和演示视频继续按阶段完成。最新测试与浏览器验证见[第四阶段记录](07-stage-four.md)，前几阶段文档保留历史记录。
