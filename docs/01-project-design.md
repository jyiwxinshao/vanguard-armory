...
本方案以一个人完成课程开发、测试、文档和答辩为前提。第一版围绕游戏装备浏览、购买和交付形成完整流程，优先保证数据正确和页面可用；基础验收通过后，再选择少量视觉或交互扩展。任务书仅作参考，明显错误和无关内容直接忽略，具体功能按游戏场景与单人开发规模确定。本文描述完整设计；当前工程基础、认证、装备浏览与购物车已接入，具体范围与验证状态见[第四阶段记录](07-stage-four.md)。
...
首页主要桌面布局为四列装备卡片，随屏幕变窄依次调整为三列、两列，必要时一列；沿用现有紧凑间距和细边框。卡片展示图片、分类、稀有度、名称、最多两个已实现属性、库存、价格及“查看详情”。当前图片仍为已有占位资源，失败后只尝试一次备用图，再显示不可用状态，避免循环请求。

顶部“我的订单”、购物车入口与数量角标属于用户当前界面的业务预留，不代表订单、购物车或购买已经可用。数量选择和加购统一在第四阶段实现；阶段三只完成浏览和详情，不假定预留按钮已接通。
