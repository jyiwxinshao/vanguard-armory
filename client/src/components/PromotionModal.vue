...
const fallbackLabel = computed(() => theme.value.toUpperCase());
...
function markFailed() { failed.value[currentIndex.value] = true; }
...
<template>
  <div class="promotion-overlay" role="presentation" @click.self="close">
    ...
    >
      ...
      <div class="promotion-poster" :class="{ 'is-fallback': failed[currentIndex] }">
        <img v-if="!failed[currentIndex]" :src="current.image" :alt="current.series || '新系列宣传'" @error="markFailed">
        <span v-else class="promotion-fallback-mark" aria-hidden="true">{{ fallbackLabel }}</span>
      </div>
