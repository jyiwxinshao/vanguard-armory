      ...
      <div v-else-if="orders.confirmation.length" class="order-layout">
        ...
        <form class="order-panel auth-form" novalidate @submit.prevent="submit">
          ...
          <div class="checkout-character" aria-live="polite">
            ...
            <div v-else>
              <p class="auth-error">当前账号在该服务器暂无可用角色。如果管理员刚刚完成分配，可点击“刷新角色”重新获取。</p>
              <button type="button" class="cart-toolbar-button" :disabled="orders.characterLoading" @click="orders.loadCharacter(form.server)">刷新角色</button>
            </div>
